...
try:
    ...
    class EnhancedBandpass:
        ...
        def set_notch_enabled(
            self, enabled: bool): self.use_notch = bool(enabled)
...
class ECGController:
    def __init__(
        ...
        self.mode = str(plot_cfg.get("mode", "sweep")
                        ).strip().lower()  # "sweep" | "sliding"
        self.direction = str(plot_cfg.get("direction", "ltr")
                             ).strip().lower()  # 只在 sliding 用
        ...
        self.curve = self.plot.plot(
            self._tbase, self._ybuf, pen=pg.mkPen(width=2))
        ...
        # ← 新增：從 config.toml 讀 y_padding
        plot_cfg = cfg.get("plot", {})
        y_pad = float(plot_cfg.get("y_padding", 0.20))  # 預設 0.20

        # ViewBox 設定：Y 自動、X 我們控制
        vb = self.plot.getViewBox()
        vb.setDefaultPadding(y_pad)  # ★ 上下留白由 config 控制
        vb.enableAutoRange(axis=pg.ViewBox.YAxis, enable=True)
        vb.enableAutoRange(axis=pg.ViewBox.XAxis, enable=False)
        vb.invertX(False)  # 保持左→右
        ...
        self.client.configure(
            address=self.address, sampling_rate=self.fs, analog_channels=self.analog_channels)
